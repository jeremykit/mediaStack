...
| 视频格式 | MP4 |
| 音频格式 | MP3、M4A |
| 文件大小 | 支持大文件上传（分片上传），视频最大 10GB、音频最大 1GB |
| 分片大小 | 前端默认 5MB/每片，可配置 |
| 服务器请求体上限 | Nginx 需设置 `client_max_body_size` ≥ 单片大小，否则分片请求会被 Nginx 以 413 拒绝 |
| 转码处理 | 不需要，直接存储原始文件 |
| 文件类型检测 | 自动识别音频文件（MP3/M4A）并设置为音频类型 |

**功能清单：**
- 视频文件上传（MP4，支持拖拽上传）
- 音频文件上传（MP3/M4A，支持拖拽上传）
- 自动文件类型检测：
  - MP3/M4A 自动识别为音频类型
  - MP4 识别为视频类型
- 上传进度显示
...
|--------|------|
| 裁剪时机 | 录制完成后手动裁剪 |
| 裁剪方式 | 指定开始/结束时间，保留中间部分 |
| 音频提取 | 从裁剪后的视频提取音频（MP3格式） |
| 存储策略 | 默认不保留原始文件，可选保留 |
...
- 裁剪对话框功能：
  - 视频预览播放器
  - 时间轴拖动选择起止点
  - 开始/结束时间输入框（HH:MM:SS格式）
  - 实时显示裁剪后时长
  - "提取音频"复选框
  - "保留原始文件"复选框（默认不勾选）
- 裁剪处理：
  - 使用FFmpeg裁剪视频（`-ss` 和 `-to` 参数，`-c copy` 避免重新编码）
  - 可选提取音频为MP3格式（192kbps码率）
  - 默认删除原始文件，节省存储空间
  - 可选保留原始文件和裁剪版本
- 音频文件关联：
  - 提取的音频关联到视频记录
...
**裁剪规则：**
- 裁剪后文件命名：`trimmed_YYYYMMDD_HHmmss_<原文件ID>.mp4`
- 提取音频命名：`audio_YYYYMMDD_HHmmss_<原文件ID>.mp3`
- 裁剪使用 `-c copy` 避免重新编码，保证速度和质量
- 音频提取使用 `libmp3lame` 编码器，192kbps码率
...
**音频播放器特性：**
- 自动检测文件类型（MP3/M4A上传时自动识别为音频）
- 波形可视化使用 wavesurfer.js 库
...
|------|--------|
| 格式 | MP3 |
| 码率 | 192kbps（裁剪提取）/ 128kbps（普通提取） |
| 采样率 | 44100Hz |
| 声道 | 立体声 |

**音频提取场景：**
1. **视频裁剪时提取音频**：192kbps 高码率
2. **普通音频下载**：128kbps 标准码率
3. **支持格式**：MP3、M4A
...
| 请求体限制 | Nginx 反向代理需配置 `client_max_body_size 20m`，以允许 5MB 分片上传（默认 1MB 会导致 413 Content Too Large） |
